...
class DispalyHistoryScreen extends State<HistoryScreen> {
  ...
  late bool _isFirstResponder =
      false; // initiating value, will be change on _loadToken
  int _selectedIndex =
  ...
  void _getNotifications() async {
    ...
    try {
      var response = await http.get(
          ...
          Uri.parse(
              "https://rapid-response-pi.vercel.app/api/notification/latest-notifications"),
          headers: {
  ...
  Widget _buildNotificationDisplay(
    ...
    if (notificationType == "emergency-contact-remove") {
      ...
      // creating an else to account for other notification types unaccounted for supervisee-request
      cusIcon = const Icon(
      ...
      body: Container(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            return _buildNotificationDisplay(_notifications[index]["title"],
                _notifications[index]["body"], _notifications[index]["type"]);
          },
        ),
      ),
