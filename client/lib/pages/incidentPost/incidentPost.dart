...
class _IncidentScreenState extends State<IncidentScreen> {
  ...
  Future<void> fetchIncidents() async {
    try {
      final response = await http.get(Uri.parse('http://10.0.2.2:3000/api/posts/incidents/latest'));
      if (response.statusCode == 200) {
        final List<dynamic> jsonData = json.decode(response.body);
        setState(() {
          incidents = jsonData.map((item) => Incident(
            title: item['title'],
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: incidents.isNotEmpty ? ListView.builder(
