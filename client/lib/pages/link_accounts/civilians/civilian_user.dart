...
class _CivilianUserPageState extends State<CivilianUserPage> {
  ...
  isEmergencyContact() async {
    try {
      var response = await http.get(
          Uri.parse(
              "https://rapid-response-pi.vercel.app/api/linked-accounts/emergency-contacts/${widget._user["_id"]}"),
          headers: {
            'Content-Type': 'application/json',
            if (_accessToken != null) 'x-auth-token': _accessToken,
          });
  ...
  isRequestSent() async {
    try {
      var response = await http.get(
          Uri.parse(
              "https://rapid-response-pi.vercel.app/api/notification/search/request/${widget._user["_id"]}?type=emergency-contact-request"),
          headers: {
            'Content-Type': 'application/json',
            if (_accessToken != null) 'x-auth-token': _accessToken,
          });
